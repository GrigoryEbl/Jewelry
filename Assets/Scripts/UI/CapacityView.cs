...
{
    internal class CapacityView : MonoBehaviour, IUpgraderViewer
    {
        ...
        [SerializeField] private HandChanger _capacityChanger;
        [SerializeField] private Image _imageMiddleDetail;
        [SerializeField] private Image _imageHighDetail;

        public void OnInfoChange()
        {
            ChangeText(_priceText, _upgrader.PriceUpgradeCapacity, _levelText, _car.CapacityLevel, _label);
            Slide(_slider, _car.CapacityLevel);
            SetImageDetail(_car.CapacityLevel, _capacityChanger.LevelToMiddleCapacity, _imageMiddleDetail, _imageHighDetail);
        }
